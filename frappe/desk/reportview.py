...
def get_count() -> int:
    ...
    if is_virtual_doctype(args.doctype):
        ...
        if args.limit:
            ...
            count = frappe.db.sql(
                f"""select count(*) from ( {partial_query} ) p""")[0][0]
        else:
...
def raise_invalid_field(fieldname):
    frappe.throw(_("Field not permitted in query") +
                 f": {fieldname}", frappe.DataError)
...
def save_report(name, doctype, report_settings):
    ...
    if frappe.db.exists("Report", name):
        ...
        if report.report_type != "Report Builder":
            frappe.throw(
                _("Only reports of type Report Builder can be edited"))
...
def export_query():
    ...
    add_totals_row = 1 if form_params.pop(
        "add_totals_row", None) == "1" else None
    ...
    if file_format_type == "CSV":
        ...
        content = get_csv_bytes(
            [[handle_html(frappe.as_unicode(v)) if isinstance(
                v, str) else v for v in r] for r in data],
            csv_params,
        )
    ...
    for field in fields:
        ...
        if df and df.fieldtype == "Duration":
            ...
            for i in range(1, len(data)):
                ...
                if val_in_seconds:
                    duration_val = format_duration(
                        val_in_seconds, df.hide_days)
            ...
            if stats[tag["name"]]:
                data = [
                    ...
                    frappe.get_list(
                        ...
                        fields=[tag["name"], "count(*)"],
                        filters=[
                            *filters, "({0} = '' or {0} is null)".format(tag["name"])],
                        as_list=True,
                    )[0][1],
                ]
...
def build_match_conditions(doctype, user=None, as_condition=True):
    match_conditions = DatabaseQuery(
        doctype, user=user).build_match_conditions(as_condition=as_condition)
    if as_condition:
